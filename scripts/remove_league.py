import os
from event_store_v2 import load_events, save_events, sort_leagues, prune_empty_leagues, resync_channel_links
...
def main():
    ...
    if removed_count:
        ...
        save_events(data)
        Logger.success(f"Removed league {target_id} and its events from events_v2.json.")
    else:
        Logger.info(f"League {target_id} had no entry in events_v2.json. Nothing to remove there.")
