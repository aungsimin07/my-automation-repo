...
LOAD_CONTROL_KEY_MAP = {
    "load-control-min-buffer-ms": "minBufferMs",
    "load-control-max-buffer-ms": "maxBufferMs",
    "load-control-buffer-for-playback-ms": "bufferForPlaybackMs",
    "load-control-buffer-for-playback-after-rebuffer-ms": "bufferForPlaybackAfterRebufferMs",
}
...
def apply_option_line(entry: dict, tag: str, key: str, value: str):
    if tag == "EXTVLCOPT":
        ...
        elif key == "is-server-dead":
            entry["isServerDead"] = parse_bool(value)
        elif key in LOAD_CONTROL_KEY_MAP:
            if value.isdigit():
                load_control = entry.setdefault("loadControl", {})
                load_control[LOAD_CONTROL_KEY_MAP[key]] = int(value)
...
def filter_dead_servers(entries: list) -> list:
    """Drop entries marked #EXTYTVMETA:is-server-dead=true — the stream
    is known down, so there's no point tracking it or, downstream, using
    its channelPath to scrape for upcoming events."""
    alive = []
    dropped = 0
    for entry in entries:
        if entry.get("isServerDead"):
            dropped += 1
            Logger.info(f"Skipping dead server entry: '{entry.get('title')}' ({entry.get('url')})")
        else:
            alive.append(entry)
    if dropped:
        Logger.info(f"Filtered out {dropped} dead-server entr(y/ies).")
    return alive
